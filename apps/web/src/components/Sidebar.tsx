...
import {
  isTerminalThread,
  newTerminalThreadId,
  DEFAULT_TERMINAL_THREAD_TITLE,
} from "../lib/terminalThread";
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: SidebarThreadRowProps) {
  ...
  const isPinned = useUiStateStore((state) => state.pinnedThreadKeys.has(threadKey));
  ...
  const threadMetaClassName = isConfirmingArchive
    ...
    : !isThreadRunning
      ? "pointer-events-none transition-opacity duration-150 max-[599px]:pr-12 group-hover/menu-sub-item:opacity-0 group-focus-within/menu-sub-item:opacity-0"
      : "pointer-events-none";
        ...
        <div className="ml-auto flex shrink-0 items-center gap-1.5">
          ...
          <div
            className={`flex min-w-12 justify-end ${
              isRemoteThread ? "max-[599px]:min-w-28" : "max-[599px]:min-w-24"
            }`}
          >
            {isConfirmingArchive ? (
              <button
              ...
              <div className="pointer-events-none absolute top-1/2 right-1 flex -translate-y-1/2 items-center gap-0.5 opacity-0 transition-opacity duration-150 max-[599px]:pointer-events-auto max-[599px]:opacity-100 group-hover/menu-sub-item:pointer-events-auto group-hover/menu-sub-item:opacity-100 group-focus-within/menu-sub-item:pointer-events-auto group-focus-within/menu-sub-item:opacity-100">
                <Tooltip disabled={isRenaming}>
                  <TooltipTrigger
                    render={
                      <button
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const toggleThreadPinnedAction = useUiStateStore((state) => state.toggleThreadPinned);
  ...
  const pinnedThreadIds = useMemo(() => {
    if (pinnedThreadKeys.size === 0) return pinnedThreadKeys;
    const ids = new Set<string>();
    for (const thread of projectThreads) {
      const key = scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id));
      if (pinnedThreadKeys.has(key)) ids.add(thread.id);
    }
    return ids;
  }, [pinnedThreadKeys, projectThreads]);
  ...
  }, [projectThreads, threadLastVisitedAts, threadSortOrder, pinnedThreadIds]);
  ...
  const handleCreateTerminalClick = useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      event.preventDefault();
      event.stopPropagation();
      ...
      const threadId = newTerminalThreadId();
      const title = DEFAULT_TERMINAL_THREAD_TITLE;

      const threadRef = scopeThreadRef(member.environmentId, threadId);
      ...
      void (async () => {
        try {
          ...
          // Wait for the thread to appear in the store before navigating
          await new Promise<void>((resolve) => {
            if (selectThreadByRef(useStore.getState(), threadRef)) {
              resolve();
              return;
            }
            ...
            // Timeout fallback — navigate anyway after 2s
            setTimeout(() => {
              unsubscribe();
              resolve();
            }, 2_000);
    ...
    [navigateToThread, project.memberProjects],
  );
  ...
  const handleThreadContextMenu = useCallback(
    async (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      ...
      const isCurrentlyPinned = pinnedThreadKeys.has(threadKey);
      const clicked = await api.contextMenu.show(
        [
          { id: "toggle-pin", label: isCurrentlyPinned ? "Unpin thread" : "Pin thread" },
      ...
      if (clicked === "toggle-pin") {
        toggleThreadPinnedAction(threadKey);
        return;
      }
    ...
    <>
      <div className="group/project-header relative">
        <SidebarMenuButton
          ...
          size="sm"
          className={`gap-2 px-2 py-1.5 pr-8 text-left hover:bg-accent group-hover/project-header:bg-accent group-hover/project-header:text-sidebar-accent-foreground max-[599px]:pr-14 ${
        ...
        {/* Environment badge – visible by default, hides on hover when
            the terminal button appears. Positioned to the left of the
            ...
            <TooltipTrigger
              render={
                <span
                  ...
                  className="pointer-events-none absolute top-1 right-8 inline-flex size-5 items-center justify-center rounded-md text-muted-foreground/50 transition-opacity duration-150 group-hover/project-header:opacity-0 group-focus-within/project-header:opacity-0 max-[599px]:group-hover/project-header:opacity-100 max-[599px]:group-focus-within/project-header:opacity-100"
                />
              }
            >
        ...
        <div className="absolute top-1 right-1.5 flex items-center gap-0.5">
          <div className="pointer-events-none flex items-center gap-0.5 opacity-0 transition-opacity duration-150 max-[599px]:pointer-events-auto max-[599px]:opacity-100 group-hover/project-header:pointer-events-auto group-hover/project-header:opacity-100 group-focus-within/project-header:pointer-events-auto group-focus-within/project-header:opacity-100">
            <Tooltip>
              <TooltipTrigger
                render={
                  <button
                    type="button"
                    aria-label={`New terminal in ${project.displayName}`}
        ...
        <TooltipTrigger
          render={
            ...
            >
              ...
              <span
                className={`rounded-full px-1.5 py-0.5 text-[8px] font-medium uppercase tracking-[0.18em] ${APP_STAGE_LABEL === "Dev" ? "bg-primary/15 text-primary/80" : "bg-muted/50 text-muted-foreground/60"}`}
              >
                {APP_STAGE_LABEL}
              </span>
            </Link>
...
export default function Sidebar() {
  ...
  const sidebarPinnedThreadKeys = useUiStateStore((store) => store.pinnedThreadKeys);
  const navigate = useNavigate();
  ...
  const expandedThreadListsByProject = useUiStateStore(
    (state) => state.threadListExpandedByProject,
  );
  const expandThreadListForProject = useUiStateStore((state) => state.expandThreadListForProject);
  const collapseThreadListForProject = useUiStateStore(
    (state) => state.collapseThreadListForProject,
  );
  ...
  const visibleSidebarThreadKeys = useMemo(
    () =>
      sortedProjects.flatMap((project) => {
        const rawProjectThreads = (threadsByProjectKey.get(project.projectKey) ?? []).filter(
          (thread) => thread.archivedAt === null,
        );
        const projectPinnedIds =
          sidebarPinnedThreadKeys.size > 0
            ? new Set(
                rawProjectThreads
                  .filter((t) =>
                    sidebarPinnedThreadKeys.has(
