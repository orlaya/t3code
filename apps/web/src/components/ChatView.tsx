...
const PersistentThreadTerminalDrawer = memo(function PersistentThreadTerminalDrawer({
  ...
  return (
    <div
      className={cn(
        visible
          ? isTerminalOnlyThread
            ? "flex min-h-0 flex-1 flex-col [&>.thread-terminal-drawer]:!h-full"
            : undefined
          : "hidden",
      )}
    >
...
export default function ChatView(props: ChatViewProps) {
  ...
  const onSelectActivePendingUserInputOption = useCallback(
    ...
    [composerRef, activePendingProgress?.activeQuestion, activePendingUserInput],
  );
  ...
  const onChangeActivePendingUserInputCustomAnswer = useCallback(
    ...
    [composerRef, activePendingUserInput],
  );
  ...
  const onCopyTurnJson = useCallback(
    (turnId: TurnId) => {
      ...
      const json = JSON.stringify(
        { turnId, messages: turnMessages, activities: turnActivities },
        null,
        2,
      );
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      {mountedTerminalThreadRefs.map(({ key: mountedThreadKey, threadRef: mountedThreadRef }) => (
        <PersistentThreadTerminalDrawer
          ...
          visible={
            mountedThreadKey === activeThreadKey && (isTerminalOnly || terminalState.terminalOpen)
          }
          launchContext={
