...
describe("getTimestampFormatOptions", () => {
  it("omits hour12 when locale formatting is requested", () => {
    expect(getTimestampFormatOptions("locale")).toEqual({
      hour: "numeric",
      minute: "2-digit",
    });
  });

  it("builds a 12-hour formatter", () => {
    expect(getTimestampFormatOptions("12-hour")).toEqual({
      hour: "numeric",
      minute: "2-digit",
      hour12: true,
    });
  ...
  it("builds a 24-hour formatter", () => {
    expect(getTimestampFormatOptions("24-hour")).toEqual({
      hour: "numeric",
      minute: "2-digit",
      hour12: false,
    });
