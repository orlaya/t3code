  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    if (event.payload.customSlashCommand) {
      ...
      if (cmdEntry.promptFile) {
        const promptFileContent = yield* fs
          .readFileString(cmdEntry.promptFile, "utf-8")
          .pipe(Effect.option);
        if (Option.isNone(promptFileContent)) {
          yield* appendProviderFailureActivity({
            threadId: event.payload.threadId,
            kind: "provider.turn.start.failed",
            summary: "Custom slash command failed",
        ...
        promptParts.push(promptFileContent.value.trim());
      }
