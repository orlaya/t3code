...
const buildAppUnderTest = (options?: {
  ...
  Effect.gen(function* () {
    ...
    // This layer has 22 `Layer.provide(...)` arguments. `pipe()`'s typed
    // overloads cap at 20 args, so the chain MUST stay split across two
    // `.pipe()` calls — merging into one is a hard TS2554 that collapses the
    // layer to `never` and triggers a ~200-error cascade. The rule can't see
    // the arity limit, so this "simplification" is a false positive here.
    // @effect-diagnostics-next-line effect/unnecessaryPipeChain:off
