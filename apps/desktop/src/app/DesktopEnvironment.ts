...
const makeDesktopEnvironment = Effect.fn("desktop.environment.make")(function* (
  ...
  const baseDir = Option.getOrElse(config.t3Home, () => path.join(homeDirectory, ".t3-tweaked"));
  ...
  const userDataDirName = isDevelopment ? "t3code-dev" : "t3code-tweaked";
  const legacyUserDataDirName = isDevelopment ? "T3 Code (Dev)" : "T3 Code (Tweaked)";
  ...
  return DesktopEnvironment.of({
    ...
    branding,
    displayName,
    appUserModelId: isDevelopment ? "com.t3tools.t3code.dev" : "com.t3tools.t3code.tweaked",
    linuxDesktopEntryName: isDevelopment ? "t3code-dev.desktop" : "t3code-tweaked.desktop",
    linuxWmClass: isDevelopment ? "t3code-dev" : "t3code-tweaked",
    userDataDirName,
    ...
    developmentDockIconPath: path.join(rootDir, "assets", "dev", "t3-tweaks-dev-mac.png"),
  });
});
