...
- `apps/web/src/uiStateStore.ts` — added `threadListExpandedByProject: Set<string>` to `UiProjectState` (was local `useState` in Sidebar.tsx). `toggleProject` store action now does accordion: expanding a project immediately opens it, then collapses all others after a 100ms staggered delay (`ACCORDION_COLLAPSE_DELAY_MS`) so the close animation feels less abrupt. Collapsing a project is still instant. `setProjectExpanded` uses the same stagger. Both also collapse thread lists for auto-closed projects. New `expandThreadListForProject` / `collapseThreadListForProject` actions on the store.
- `apps/web/src/components/Sidebar.tsx` — removed local `expandedThreadListsByProject` useState + the two useCallback wrappers. Now reads `threadListExpandedByProject`, `expandThreadListForProject`, `collapseThreadListForProject` from `useUiStateStore`. Prop types unchanged (`(projectKey: string) => void`).
