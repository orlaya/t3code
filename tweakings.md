...
- `apps/web/src/components/chat/MessagesTimeline.tsx` — new `SubAgentDetailDialog` component. Heading row mirrors the pinned entry style (icon + bold heading + detail, inside a status-colored card). Brief prompt text sits below unbounded. Work log section shows `SimpleWorkEntryRow` entries filtered by matching `taskId`, wrapped in the standard work log box (`rounded-lg border border-border/45 bg-card/25`). Response section uses the same divider pattern as the main chat completion divider (horizontal lines with a "Response" pill). Response text rendered via `ChatMarkdown` at `text-[13px] leading-snug`. Dialog uses `max-w-md max-h-[75vh]`.
