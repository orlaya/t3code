...
- `packages/contracts/src/keybindings.ts` — added `"search.toggle"` to `STATIC_KEYBINDING_COMMANDS`.
- `apps/server/src/keybindings.ts` — added default `{ key: "mod+f", command: "search.toggle" }`. No `when` clause — scoping handled at the component level.
- `apps/web/src/keybindings.ts` — added `isSearchToggleShortcut()` helper using `matchesCommandShortcut`.
- `apps/web/src/components/SearchOverlay.tsx` — **new file**. Reusable floating Chrome-style search bar (top-right, `w-72` fixed). Input + match counter (`tabular-nums`, `whitespace-nowrap`) + prev/next chevrons + close button. Enter/Shift+Enter cycle matches. Escape closes. Buttons have `tabIndex={-1}` so Tab doesn't leave the input. Exports `SearchHandler` interface for future terminal search integration.
...
- `apps/web/src/components/chat/MessagesTimeline.tsx` — `hasMatchOutsideVisibleBounds()` helper: walks text nodes in a clipped container, finds query matches via `indexOf`, checks each match's `getBoundingClientRect()` against the container's visible bounds. Returns true only when a match is fully outside the visible rect.
- `apps/web/src/components/ChatView.tsx` — `searchOpen` state + `search.toggle` command handling in keyboard handler. Passes `searchOpen`/`onSearchClose` to `MessagesTimeline`.
