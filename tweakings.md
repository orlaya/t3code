...
- `apps/web/src/session-logic.ts` — subagent deduplication: when a `tool.completed` exists for a subagent, the corresponding `tool.updated` entry is now filtered out entirely (was previously kept and just marked as not-in-progress). The completed entry supersedes it and carries both `subAgentBrief` and `subAgentResult`.
...
- `apps/web/src/components/chat/MessagesTimeline.tsx` — `PinnedSubAgentEntry` changed from a `div` to a `button`, clickable when `subAgentBrief` is present. Opens a `SubAgentDetailDialog` modal.
- `apps/web/src/components/chat/MessagesTimeline.tsx` — new `SubAgentDetailDialog` component. Heading row mirrors the pinned entry style (icon + bold heading + detail, inside a status-colored card). Brief and work log sections are both collapsible (closed by default) with clickable header + chevron up/down toggle. Brief clamps to `max-h-[12.5em]` (~8 lines) when collapsed with a bottom gradient fade; entire card is the click target. Work log shows last 4 `SimpleWorkEntryRow` entries when collapsed (same `MAX_VISIBLE_WORK_LOG_ENTRIES` as main chat), all entries when expanded, filtered by matching `taskId`, wrapped in the standard work log box (`rounded-lg border border-border/45 bg-card/25`). Response section uses the same divider pattern as the main chat completion divider. Response text rendered via `ChatMarkdown` at `text-[12.5px] leading-snug`. Dialog uses `max-w-lg max-h-[75vh]`.
...
- `apps/web/src/components/ui/dialog.tsx` — close button repositioned from `end-2 top-2` to `end-1.5 top-1.5` (tighter to corner). Size changed from `size="icon"` (36/32px) to `size="icon-sm"` (32/28px) with explicit `size-4` on XIcon. More compact for desktop use.
